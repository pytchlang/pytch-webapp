...
const dummyProjectSummaries: Array<IProjectSummary> = [
  {
    id: 1001,
    name: "Bash the zombies",
    summary: "Run round splatting zombies.",
  },
...
const initialProjectContent = (summary: IProjectSummary): IProjectContent => {
  return {
    ...
    assets: [
      { name: "banana.png", mimeType: "image/png", id: "a1" },
...
const hexOfBuffer = (data: ArrayBuffer): string => {
  ...
  for (let i = 0; i !== u8s.length; ++i) octetStrings[i] = octetString[u8s[i]];
