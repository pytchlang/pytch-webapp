...
export const bootApi = (): GoogleDriveBootApi => {
  // TODO: Document REACT_APP_USE_REAL_GOOGLE_DRIVE env.var.  Set in
  // launch script?  How to override for running Cypress tests?  How to
  // skip the Google drive tests when running against deployment zipfile
  // via Docker?
  //
  ...
  if (
    ...
    process.env.REACT_APP_USE_REAL_GOOGLE_DRIVE === "yes"
  ) {
    return realBootApi;
  } else {
    return mockBootApi;
